  ...
  Serial.println("Done!");
    ...
    while(true){
      ...
      if ((millis() - ultimo_millis2) > debounce_delay) { // se ja passou determinado tempo que o botao foi precionado
        ...
        Serial.print("POT_1-> ");
        Serial.println(dimLight->getBrightness()); // mostra a quantidade de brilho atual
      ...
      if (tempATUAL<tempPROG - 1.0 && linha_1){
              potencia_1 = 0;
              dimLight = dlm.get("DIMMER_1");
              dimLight->setBrightness(map(potencia_1, 0, 100, minPot, maxPot));
              ligaRELE(RELE_1);
      }else if(linha_1){
        desligaRELE(RELE_1);
        if (tempATUAL != lastTempATUAL){
          if(tempATUAL < lastTempATUAL && tempATUAL < tempPROG-0.0){
              potencia_1 = potencia_1 + ADJUSTABLE;
